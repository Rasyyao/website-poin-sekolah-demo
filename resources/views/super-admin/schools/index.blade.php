...
<div class="flex items-center justify-between mb-6">
    <form method="GET" class="flex gap-3">
        <div x-data="{ showModal: {{ (old('_method') !== 'PUT' && $errors->any()) ? 'true' : 'false' }} }" @open-modal.window="showModal = true">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari sekolah..." class="px-4 py-2 rounded-lg bg-white border border-slate-200 text-slate-900 text-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 w-64">
        </div>
        <button type="submit" class="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 transition-colors cursor-pointer shadow-sm">Cari</button>
    </form>
    <a href="{{ route('super-admin.schools.create') }}" class="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 transition-colors">+ Tambah Sekolah</a>
</div>
    ...
    <div class="overflow-x-auto">
        ...
        <tbody class="divide-y divide-slate-100">
            @forelse($schools as $school)
                <tr class="hover:bg-whiteer/30 transition-colors group" x-data="{ showEditModal: {{ (old('_method') == 'PUT' && old('school_id') == $school->id && $errors->any()) ? 'true' : 'false' }} }">
                    <td class="px-6 py-4 text-sm text-slate-900 font-medium group-hover:text-blue-600 transition-colors">{{ $school->name }}</td>
                    <td class="px-6 py-4 text-xs text-slate-500 font-mono">{{ $school->slug }}</td>
                                    ...
                                    <div class="bg-slate-100 rounded-xl shadow-2xl border border-slate-200 w-full max-w-lg max-h-[90vh] overflow-y-auto p-6" @click.stop>
                                        ...
                                        <form method="POST" action="{{ route('super-admin.schools.update', $school) }}" class="space-y-5 text-left">
                                            ...
                                            <div>
                                                ...
                                                <input type="text" name="name" value="{{ old('school_id') == $school->id ? old('name') : $school->name }}" required class="w-full px-4 py-2.5 rounded-lg bg-white border border-gray-600 text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-slate-700 mb-1.5">Slug</label>
                                                <p class="text-sm text-slate-500 font-mono">{{ $school->slug }}</p>
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-slate-700 mb-1.5">Status Langganan</label>
                                                <select name="subscription_status" class="w-full px-4 py-2.5 rounded-lg bg-white border border-gray-600 text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
