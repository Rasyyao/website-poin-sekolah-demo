...
<aside class="w-full h-full bg-white border-r border-slate-200 flex flex-col">
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        ...
        @if($role === 'super_admin')
            ...
            <a href="{{ route('super-admin.schools.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('super-admin.schools.*') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
        ...
        @if(in_array($role, ['super_admin', 'admin', 'kesiswaan']))
            ...
            <a href="{{ route('admin.reports.dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.reports.dashboard') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
            ...
            <a href="{{ route('admin.reports.ranking') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.reports.ranking') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
            ...
            @if(in_array($role, ['super_admin', 'admin']))
                <a href="{{ route('admin.staff.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.staff.*') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
            ...
            <a href="{{ route('admin.students.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.students.*') && ! request()->routeIs('admin.students.migration') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"/></svg>
                Siswa
            </a>
            <a href="{{ route('admin.classes.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.classes.*') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
            ...
            <a href="{{ route('admin.students.migration') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.students.migration') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
            ...
            <a href="{{ route('admin.rules.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.rules.*') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25"/></svg>
                Peraturan
            </a>
            @if(in_array($role, ['super_admin', 'admin']))
                <a href="{{ route('admin.rule-thresholds.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.rule-thresholds.*') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"/></svg>
                    Ambang Batas
                </a>
            @endif
            @if(in_array($role, ['super_admin', 'admin', 'kesiswaan']))
                <a href="{{ route('admin.certificates.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.certificates.*') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
            ...
            <a href="{{ route('admin.points-log.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->requestUri === route('admin.points-log.index', [], false) ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
            ...
            <a href="{{ route('admin.appeals.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.appeals.*') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
        ...
        @if(in_array($role, ['teacher', 'homeroom', 'counselor']))
            ...
            <a href="{{ route('teacher.my-students') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('teacher.my-students') || request()->routeIs('teacher.students.*') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
        ...
        @if(in_array($role, ['super_admin', 'admin']))
            ...
            <a href="{{ route('admin.academic-years.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors {{ request()->routeIs('admin.academic-years.*') ? 'bg-blue-500/10 text-blue-500 font-medium' : 'text-slate-500 hover:bg-slate-500/10 hover:text-slate-900' }}">
    ...
    <div class="hidden lg:block border-t border-slate-200 p-3 mt-auto shrink-0">
        <div class="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-slate-50 transition-colors group">
            {{-- Avatar --}}
            <div class="w-9 h-9 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400 font-bold text-sm border border-blue-500/30 shrink-0 shadow-sm">
