...
<div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
    <div class="p-4 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50">
        ...
        <div class="relative w-full sm:w-auto">
               ...
               class="pl-9 pr-4 py-2 rounded-lg bg-white border border-slate-200 text-slate-900 text-sm placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 w-48 sm:w-64">
            </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tr class="bg-whiteer/50">
                ...
                <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Total Poin</th>
            ...
            @forelse($students as $student)
                <tr class="hover:bg-whiteer/30 transition-colors group" x-data="{ showEditModal: {{ (old('_method') == 'PUT' && old('student_id') == $student->id && $errors->any()) ? 'true' : 'false' }} }">
                    ...
                    <td class="px-6 py-4 text-sm text-center">
                        ...
                        <div class="inline-flex items-center justify-center px-3 py-1 rounded-full bg-slate-100 text-sm font-bold border {{ $tp > 0 ? 'border-red-500/20 text-red-400' : ($tp < 0 ? 'border-green-500/20 text-green-400' : 'border-slate-200 text-slate-500') }}">
                            {{ abs($tp) }}
                        </div>
                    </td>
