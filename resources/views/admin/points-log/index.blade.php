        ...
        <tbody class="divide-y divide-slate-100">
            @forelse($logs as $log)
                <tr class="hover:bg-whiteer/30 transition-colors group">
                    ...
                    <td class="px-6 py-4 text-sm text-slate-900 font-medium group-hover:text-blue-600 transition-colors">{{ $log->student->name }}</td>
                    <td class="px-6 py-4 text-sm text-slate-700">{{ $log->rule->name }}</td>
                    <td class="px-6 py-4 text-center">
                        <div class="inline-flex items-center justify-center px-3 py-1 rounded-full bg-slate-100 text-sm font-bold border {{ $log->points < 0 ? 'border-red-500/20 text-red-400' : 'border-green-500/20 text-green-400' }}">
