...
<div class="bg-surface-light rounded-2xl border border-gray-700/50 overflow-hidden shadow-sm">
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <th class="px-6 py-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Alasan</th>
        ...
        <tbody class="divide-y divide-gray-700/30">
            @forelse($appeals as $appeal)
                <tr class="hover:bg-surface-lighter/30 transition-colors group">
                    <td class="px-6 py-4 text-xs text-gray-400">{{ $appeal->created_at->format('d/m/Y H:i') }}</td>
                    <td class="px-6 py-4 text-sm text-gray-200 font-medium group-hover:text-primary-300 transition-colors">{{ $appeal->pointsLog->student->name ?? '-' }}</td>
                    <td class="px-6 py-4 text-sm text-gray-300">{{ $appeal->pointsLog->rule->name ?? '-' }}</td>
                    <td class="px-6 py-4 text-sm text-gray-300 max-w-xs truncate" title="{{ $appeal->reason }}">{{ $appeal->reason }}</td>
