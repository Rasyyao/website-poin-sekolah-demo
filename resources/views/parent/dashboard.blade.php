...
@section('nav-links')
    <a href="{{ route('parent.report') }}" class="text-sm text-gray-400 hover:text-gray-200">Laporan</a>
@endsection

@section('content')
<div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
        <h1 class="text-2xl font-bold text-gray-100">Laporan Perilaku Anak</h1>
        <p class="text-sm text-gray-400 mt-1">{{ $student->name }} — NISN: {{ $student->nisn }} — Kelas: {{ $student->currentClass?->name ?? '-' }}</p>
    </div>
    <a target="_blank" href="{{ route('parent.export.pdf') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-500 text-white text-sm font-medium rounded-xl transition-all shadow-lg shadow-green-500/20">
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
    <div class="bg-surface-light rounded-xl border border-gray-700/50 p-4 text-center">
        ...
        <p class="text-2xl font-bold mt-1 {{ $stats['total_points'] < 0 ? 'text-red-400' : 'text-green-400' }}">{{ $stats['total_points'] }}</p>
    </div>
    <div class="bg-surface-light rounded-xl border border-gray-700/50 p-4 text-center">
...
<div class="bg-surface-light rounded-2xl border border-gray-700/50 overflow-hidden shadow-sm">
    ...
    <table class="w-full text-left border-collapse">
        <thead><tr class="bg-surface-lighter/50">
            ...
            <th class="px-6 py-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Keterangan</th>
                ...
                <tr>
                    <td colspan="3" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center text-gray-500">
                            ...
                            <p class="text-base font-medium">Belum ada aktivitas.</p>
                        </div>
                    </td>
                </tr>
            @endforelse
